...
LocalizationManager::LocalizationManager(const OccupancyGrid& ogrid):ogrid(ogrid) {
...
void LocalizationManager::createNewParticles(Particle* par,vector<Particle *>* newParticles) {
	for(int i = -1;i<2;i+=2)
	{
		for(int j = -1;j<2;j+=2)
		{
			...
			{
				if(this->ogrid.getCell((par->x+i),(par->y+j))==CELL_FREE)
