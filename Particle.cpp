...
float Particle::update(HamsterAPI::LidarScan scan, const OccupancyGrid& ogrid)
{
	...
	for (size_t i = 0; i < scan.getScanSize(); i++)
	{
		...
		if (scan.getDistance(i) < scan.getMaxRange() - 0.001)
		{
			...
			double obsX = this->x * ogrid.getResolution()
					+ scan.getDistance(i)
							* cos(angle + this->teta * DEG2RAD + 180 * DEG2RAD);
			double obsY = this->y * ogrid.getResolution()
					+ scan.getDistance(i)
							* sin(angle + this->teta * DEG2RAD + 180 * DEG2RAD);
