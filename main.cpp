...
void paintInNewMap(int x, int y, cv::Mat m, int radius, int color)
{
	for(int i = x-radius; i<= x+radius; i++)
	{
		for (int j = y-radius; j<= j + radius; j++)
		{
			if (safe_paint(m,i,j))
			{
				m.at<int>(i, j, 0) = color;
			}
		}
	}
}
...
int evenDerechB(OccupancyGrid& ogrid, int robotsize)
{
	try
	{
		...
		int newHight = ogrid.getHeight() * rad;
		cv::Mat m = cv::Mat(newWidth, newHight, CV_8UC1);

		for (int i = 0; i < ogrid.getWidth(); i += 1)
		{
			for (int j = 0; j < ogrid.getHeight(); j += 1)
			{
				if (ogrid.getCell(i, j) != CELL_FREE)
				{
					paintInNewMap(i,j,m,rad,ogrid.getCell(i, j));
				}
			}
		}
